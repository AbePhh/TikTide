...
系统当前采用前后端分离 + 单体模块化架构实现，后端统一由一个 Gin HTTP 服务对外提供接口，在代码层按用户、视频、互动、Feed 等业务模块进行拆分。
...
- 终端层：Web 页面，负责登录、上传、刷关注流、互动操作
- 接入层：Gin HTTP Server，对外暴露 RESTful API 与 Swagger 文档
- 业务层：User、Video、Interact、Feed 等模块化业务目录
- 基础设施层：MySQL、Redis、Kafka、MinIO/OSS、FFmpeg

## 三、项目目录结构

当前后端目录采用“单体入口 + internal 模块化”结构，目录建议如下：
...
|Web 框架|Gin|提供路由、中间件、参数绑定与 HTTP 接口能力|
|日志|Zap|结构化日志输出|
...
|ORM 框架|GORM|统一数据库访问层，降低模型扩展成本，便于后续用户、视频、互动模块继续复用同一套数据访问模式|
|缓存|Redis 6.x|热点数据、互动状态、Feed inbox/outbox、未读数缓存|
|消息队列|Kafka 3.x|转码任务、Feed 分发、互动异步落库、通知事件|
|对象存储|阿里云 OSS|当前版本使用杭州地域 OSS，负责视频原文件直传、后续转码产物与封面资源存储|
...
- `bucket=tiktide-video`
- `OSS_ACCESS_KEY_ID` 与 `OSS_ACCESS_KEY_SECRET` 通过环境变量注入

## 五、服务边界与核心数据流

### 5.1 用户与关注关系

- `t_user`、`t_user_stats`、`t_relation` 归 User 模块管理
- 当前已实现用户注册、登录、退出登录、资料维护、改密与主页查询
- 关注关系接口与数据结构保留在文档中，后续在单体中作为独立模块补充实现

### 5.2 视频发布与转码

- `t_video`、`t_video_resource`、`t_draft`、`t_hashtag`、`t_video_hashtag` 归 Video 模块管理
- 客户端先申请上传凭证，再将原视频直传到 OSS
- 客户端调用发布接口提交 `object_key` 与业务元数据
- 当前版本已实现 `POST /api/v1/video/upload-credential` 和 `POST /api/v1/video/publish`
- 当前版本已实现 `POST /api/v1/hashtag`、`GET /api/v1/hashtag/{hid}` 与 `GET /api/v1/hashtag/{hid}/videos`
- 发布视频时支持传入 `hashtag_names`，服务端会自动创建不存在的话题
- Video 服务落库后投递 `topic.video.transcode`
- 转码任务成功后回写主表与资源表，并投递 `topic.video.ready`
- Feed 模块消费 `topic.video.ready`，将可见视频写入关注流

### 5.3 点赞、评论、收藏与通知

- `t_like`、`t_favorite`、`t_comment`、`t_comment_like`、`t_message` 归 Interact 模块管理
- 请求到达时优先更新 Redis 互动状态和计数
- 互动事件写入 Kafka 后异步批量落库
- 对需要通知的事件，写入 `topic.notify`
- Interact 模块消费者负责生成通知记录和未读数缓存

### 5.4 关注流

- Feed 模块只负责“关注流”，不负责推荐流
- 普通作者采用“有限写扩散”
...
- JWT 采用 HS256 签名，当前开发环境固定密钥为 `tiktide-system`
- Gin 中间件统一校验 Token，并将用户信息写入请求上下文
...
- 使用 Golang + Gin + GORM 搭建模块化单体短视频平台后端
